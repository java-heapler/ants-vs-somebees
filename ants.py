...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            assert isinstance(place.ant, ContainerAnt) ^ isinstance(self, ContainerAnt), 'Two ants in {0}'.format(place)
            if ContainerAnt.can_contain(self, place.ant):
                assert self.contained_ant == None, 'Two ants in {0}'.format(place)
                ContainerAnt.contain_ant(self, place.ant)
                place.ant=self
                Insect.add_to(self, place)
            elif ContainerAnt.can_contain(place.ant, self):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        and entrances_followed>=min_range and entrances_followed<=max_range
        """
        # BEGIN Problem 3 and 4
        min_range=self.min_range
        max_range=self.max_range
        entrances_followed=0
        def bee_finder(place_search=self.place):
            nonlocal entrances_followed
            if isinstance(place_search, Hive):
                return []
            if place_search.bees and entrances_followed>=self.min_range and entrances_followed<=self.max_range:
                return place_search.bees
            else:
                entrances_followed+=1
                return bee_finder(place_search.entrance)
        return random_or_none(bee_finder())
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        bees_list_copy = self.place.bees
        def damage_bees_nearby(bee_damage_amount):
            for BeeIterator in bees_list_copy[:]:
                BeeIterator.reduce_armor(bee_damage_amount)
            for BeeIterator in bees_list_copy[:]:
                if BeeIterator.armor<1: bees_list_copy.remove(BeeIterator)
        if self.armor-amount<=0:
            damage_bees_nearby(self.damage+amount)
        else:
            damage_bees_nearby(amount)
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        bees_list_copy = self.place.bees
        for BeeIterator in bees_list_copy[:]:
            BeeIterator.reduce_armor(self.damage)
        for BeeIterator in bees_list_copy[:]:
            if BeeIterator.armor<1: bees_list_copy.remove(BeeIterator)
        # END Problem 7
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        if isinstance(self, ContainerAnt) and isinstance(other, ContainerAnt) == False:
            return True
        else: return False
        # END Problem 9
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        # BEGIN Problem 10
        if self.contained_ant != None:
            Ant.action(self.contained_ant.action(gamestate), gamestate)
        bees_list_copy = self.place.bees
        for BeeIterator in bees_list_copy[:]:
            BeeIterator.reduce_armor(self.damage)
        for BeeIterator in bees_list_copy[:]:
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        bees_behind = self.place.exit.bees
        for BeeIterator in bees_behind[:]:
            if ()
            BeeIterator.damage=BeeIterator.damage*2
            douled_damage = True
        if self.armor-amount<=0:
            bees_win() """
        # END Problem 13
